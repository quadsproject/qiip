...
@pytest.mark.asyncio
async def test_reconcile_bootstraps_missing_recorder(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A fresh node without the recorder must not be treated as unreachable."""
    provisioner, ssh, store = harness
    provisioner.log_buffer.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    attempt = provisioner.log_buffer.attempts["host1"]
    store.update(attempt, status="failed", failure_summary="setup: ssh lost")
    for name in ("provision-logs.py", "log_store.py"):
        (ssh.root / "common" / name).unlink()
    # The gateway bundle is the upload source of truth (mirrors production).
    monkeypatch.setattr(provisioner, "_common_scripts_dir", lambda: ROOT / "common")
...
@pytest.mark.asyncio
async def test_reconcile_failed_collection_retains_block(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A failed collection must not clear the block from stale cached phases."""
    provisioner, ssh, store = harness
    provisioner.log_buffer.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    ...
    store.update(
        attempt,
        remote_phases={"phase-old": {"status": "complete", "exit_status": 0}},
    )
    monkeypatch.setattr(
        provisioner._remote_logs,
        "host_active",
    ...
    async def broken_collect(
        *_args: object, **_kwargs: object
        ...
        raise SSHConnectionError("host1", "read fault")

    monkeypatch.setattr(provisioner._remote_logs, "collect", broken_collect)
...
@pytest.mark.asyncio
async def test_reconcile_fresh_terminal_evidence_allows(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A successful collection proving the probed phase finished stops blocking."""
    provisioner, ssh, store = harness
    provisioner.log_buffer.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    ...
    async def fresh_collect(
        *_args: object, **_kwargs: object
    ) -> list[dict[str, object]]:
        store.update(
            attempt,
            remote_phases={"phase-new": {"status": "complete", "exit_status": 0}},
        )
        return []

    monkeypatch.setattr(provisioner._remote_logs, "collect", fresh_collect)

    assert await provisioner._reconcile_host("host1") is False


@pytest.mark.asyncio
async def test_recorder_lock_released_at_engine_handoff(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
) -> None:
    """A live engine must not hold the host mutation lock after launch."""
    provisioner, ssh, store = harness
    engine = ssh.root.parent / "fake-vllm"
    _write_executable(engine, "#!/bin/bash\nwhile :; do sleep 1; done\n")
    provisioner.log_buffer.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    attempt = provisioner.log_buffer.attempts["host1"]
    start_command = provisioner._script_command(
        "start-vllm.sh",
        env=provisioner._start_script_env("org/model", InferenceEngine.VLLM),
        scripts_dir="auto-vllm",
    )
    config = _recorder_config(
        attempt, "host1", ssh.root / "logs", command=start_command, stage="start"
    )
    await _launch(ssh, config)
    for _ in range(200):
        manifest = await _node_manifest(ssh, config)
        ...
        pytest.fail("start phase did not complete")
    engine_pid = int(Path(ssh.environment["AUTOVLLM_PID_FILE"]).read_text())
    try:
        second = store.create(
            "host1", operation="provision", bundle_version="sha256:abc"
        )
        config_b = _recorder_config(
            second,
            "host1",
        ...
        for _ in range(100):
            manifest_b = await _node_manifest(ssh, config_b)
            if manifest_b["attempt"]["status"] == "recorded":
                break
            await asyncio.sleep(0.05)
        assert "Host busy" not in json.dumps(manifest_b["attempt"]["issues"])
    finally:
...
@pytest.mark.asyncio
async def test_abandoned_launch_intent_is_closed(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
) -> None:
    """A launching row whose worker died must not block the host forever."""
    provisioner, ssh, store = harness
    attempt = store.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    node_store = AttemptLogStore(ssh.root / "logs" / "attempts.sqlite3")
    node_store.create("host1", engine="vllm", attempt_id=attempt)
    dead = subprocess.Popen(["true"])
    dead_pid = dead.pid
    dead.wait()
    node_store.update_phase(
        attempt, "phase-1", status="launching", recording=True, launcher_pid=dead_pid
    )
    config = _recorder_config(attempt, "host1", ssh.root / "logs")

    stdout, _stderr, _status = await ssh.run(
        "host1", _recorder_command(config, "active")
    )
    page = json.loads(stdout)
    assert page["active"] is False
    phase = node_store.get(attempt)["phases"]["phase-1"]
    assert phase["status"] == "complete"
    assert phase["exit_status"] == 137


@pytest.mark.asyncio
async def test_inflight_launch_intent_stays_active(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
) -> None:
    """A launching row with a live worker is still a block."""
    provisioner, ssh, store = harness
    attempt = store.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    ...
    try:
        ...
        assert page["holder"] == attempt
        assert page["status"] == "launching"
    finally:
        launcher.terminate()
        launcher.wait()
...
@pytest.mark.asyncio
async def test_reconcile_recovers_completed_remote_evidence(
    harness: tuple[NodeProvisioner, LocalNodeSSH, AttemptLogStore],
) -> None:
    """Completed work is collected even when the active probe reports idle."""
    provisioner, ssh, store = harness
    provisioner.log_buffer.create(
        "host1", engine="vllm", operation="provision", bundle_version="sha256:abc"
    )
    attempt = provisioner.log_buffer.attempts["host1"]
    config = _recorder_config(
        attempt, "host1", ssh.root / "logs", command="echo recovery-evidence"
    )
    await _launch(ssh, config)
    for _ in range(200):
        manifest = await _node_manifest(ssh, config)
        ...
        pytest.fail("remote phase did not complete")
    assert not store.get(attempt).get("remote_phases")

    assert await provisioner._reconcile_host("host1") is False
    phases = store.get(attempt)["remote_phases"]
