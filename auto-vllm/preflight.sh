...
check_model() {
    ...
    # capacity for a download when the snapshot is absent.
    ...
    if timeout --kill-after=2 "${AUTOVLLM_PROBE_TIMEOUT:-10}" test -d "${cache_dir}/hub/models--${slug}"; then
        _mark WARN "Model ${MODEL_PATH} cache directory exists but could not be verified; a download will repair it"
    fi

    if ! timeout --kill-after=2 "${AUTOVLLM_PROBE_TIMEOUT:-10}" test -d "$cache_dir" \
        || ! timeout --kill-after=2 "${AUTOVLLM_PROBE_TIMEOUT:-10}" test -w "$cache_dir"; then
