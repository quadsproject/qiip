...
| **Behavioral break** | `POST /admin/nodes/pool` self-setup adoption distinguishes a missing `/health` endpoint (HTTP 404/405/501) from an explicit unhealthy response. An authoritative unhealthy `/health` (such as 503) now refuses adoption rather than registering a HEALTHY node. A custom `port` is rejected for a plain pool registration unless `self_setup` is true, because that node is later provisioned on the configured default port. | Adopt a server only when `/health` is 200 or genuinely unavailable; an explicitly unhealthy server is not admitted. Send a custom `port` only with `"self_setup": true`. |
| **Correctness fix** | The background health checker treats a missing `/health` endpoint as optional for a self-setup node, falling back to `/v1/models`; an authoritative unhealthy `/health` response is still a failure. Managed nodes keep requiring a healthy `/health`. | Expect a self-setup node whose `/health` is 404/405/501 to remain routable as long as `/v1/models` responds; it is still demoted when `/v1/models` stops answering or `/health` returns an explicit unhealthy status. |
...
| **New surface** | The node-detail llama.cpp runtime card supersedes its read-only form with a typed automatic/custom policy editor. It seeds the persisted requested reserve and custom tuple, validates fixed limits before submission, confirms downtime, and follows relaunch task and log progress. Dirty forms become stale rather than being overwritten when another session changes the verified runtime. | Use Reset to adopt the latest persisted policy before submitting stale edits. Treat the aggregate context preview as arithmetic, not a fit guarantee; server-side estimator and post-load verification remain authoritative. |
| **New surface** | Custom llama.cpp sizing can persist `allow_estimator_overrun=true`, exposed as **Attempt launch if estimate exceeds target** in the node-detail editor. It bypasses only a valid estimator non-fit result; estimator failures, static limits, full GPU offload, and the actual post-load free-VRAM target remain fail-closed. Automatic sizing cannot enable it. | Use this only to calibrate a known-conservative estimator on an otherwise dedicated GPU. A failed attempt invokes transactional rollback. Preserve the field when replaying a previously verified custom policy, and use `effective.estimator_overrun_used` to distinguish permission from an overrun that was actually needed. |
...
| **Behavioral break** | The fleet dashboard no longer 401s anonymous visitors: `/dashboard`, `/models`, `/dashboard/tokens`, `/dashboard/users/{id}`, and `/dashboard/nodes/{id}` render a sign-in page (HTTP 200) with **Sign in with Local Admin** and **Sign in with Google Auth** choices instead. Local admin sign-in is an in-page username/password form that creates a session cookie (no native Basic challenge popup); HTTP Basic requests and the admin JSON API still work exactly as before. The admin JSON API (`/admin/*`) still 401s without valid credentials. | Scripts and monitoring unaffected. Browser users must sign in before reaching the fleet; configure `auth.session_secret` when Google sign-in should be available (the local-admin form also requires it). |
| **Behavioral break** | The admin surface now also accepts a signed-in Google user with the admin role: `require_admin_auth` (all `/admin/*` endpoints and admin-only pages) succeeds with Basic credentials **or** an admin-role session. The HTTP Basic user is the bootstrap authority and can grant/revoke roles at `POST`/`DELETE /admin/users/{id}/admin` and via the `/dashboard/admin` page. | No action required for existing Basic clients. Grant roles only to trusted Google users; a demoted user loses admin access on their next request. |
| **New surface** | `POST /admin/nodes/pool` accepts `"hidden": true` (implies `"self_setup": true`) to register a hidden inference server from an existing OpenAI-compatible URL, plus an optional operator-facing `"name"` shown in the admin fleet view. Hidden nodes are routable only to admin callers, never appear in `/fleet/nodes` or public `/v1/models`, render bold with a `hidden` badge on the admin fleet page, and are removed with the normal pool deletion endpoint. `/admin/nodes[]` gains `hidden` and `name`. | No action unless you adopt hidden servers; the hidden flag and name are additive and default to false/empty for all existing records. |
| **New surface** | `GET /fleet/nodes` returns the registered-node view for signed-in non-admins: hidden servers removed, operational actions stripped. The dashboard JS uses it for non-admin viewers while admins keep `/admin/nodes`. | Signed-in non-admin users now see the fleet; treat `/fleet/nodes` as public-within-the-org inventory. |
...
If the browser loses the relaunch POST response, the editor remains locked
until it observes a new task generation. When no new task appears, reload the
page to reconcile current node and task state before retrying. This fail-closed
behavior avoids an immediate duplicate, and the gateway lifecycle lease
rejects a concurrent relaunch if the first request was accepted.
