...
def _in_scope(
    ...
    ``None`` is the admin scope: only admin-scoped callers may route to an
    admin-only node (``owner`` ``""`` and user emails are both rejected).
    """
    if allowed_node_ids is not None and node.node_id not in allowed_node_ids:
        return False
    if node.admin_only and owner is not None:
        return False
    return not (
